...
class A14_TFN_tai(nn.Module):
    ...
    def __init__(self, args):
        ...
        '''
        super(A14_TFN_tai, self).__init__()
    ...
    def forward(self, text_x, audio_x, video_x):
        ...
        add_one = torch.ones(size=(batch_size, 1), requires_grad=False).type_as(tensor_dict[self.abl[0]]).to(
            tensor_dict[self.abl[0]].device)
        ...
        if len(self.abl) > 1:
            ...
            fusion_tensor = fusion_tensor.view(-1, (self.len_dict[self.abl[0]] + 1) * (self.len_dict[self.abl[1]] + 1),
                                               1)
        else:
            fusion_tensor = tensor_dict[self.abl].unsqueeze(-1)

        if len(self.abl) == 3:
            fusion_tensor = torch.bmm(fusion_tensor, tensor_dict[self.abl[-1]].unsqueeze(1)).view(batch_size,
                                                                                                  -1)  # bmm： 2矩阵的最后2维度，矩阵相乘
        else:
